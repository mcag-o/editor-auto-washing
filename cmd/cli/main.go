...
const usageText = "React web control plane is the primary operator surface: http://localhost:8123\nonly active operator intake is browser upload/paste with workflow/template management in the browser UI\nCLI is retained for development/debug support only; folder-intake remains backend/internal compatibility only: workspace <...> | formatting <render|validate> | rewrite <run> | automation <run-once|daemon|retry-failed|status|health|stop> [--root PATH]"
